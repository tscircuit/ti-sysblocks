...
const root = join(dirname(fileURLToPath(import.meta.url)), "..", "src", "diagrams")
...
for (const jsonFile of jsonFiles) {
  ...
  for (const id of nodeIds) {
    ...
    if (!new RegExp(`<g\\s+[^>]*id=["']${escaped}["']`).test(svg)) {
      failures.push(`${jsonFile}: node ${id} has no matching <g id="${id}"> in ${svgFile}`)
    }
  }

  if (!nodeIds.includes(source.defaultSelected)) {
    failures.push(`${jsonFile}: defaultSelected ${source.defaultSelected} is not a node ID`)
  }
}
